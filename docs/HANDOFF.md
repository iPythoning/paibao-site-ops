...
下一步方案（MCP优先）：只补CLI无法覆盖云工作台。先在既有运营官插件的MCP扩展点接站点概览、业务proposal和job结果，复用服务器端控制客户端及现有队列/权限/幂等；CLI只是可选薄入口。插件工具授权与调用者权限须核验，不自动扩大scope，默认不批准、不发布。属于多文件功能/权限接线，确认后实施，再接业务巡检→修复提案。完整goal继续active，不能仅以用户尚未登录解释缺口。

审查文档CI另发现站点测试夹具清理ENOTEMPTY：仅test/HANDOFF修复PR#10已合入，Native rm有界重试且耗尽仍失败，两条新增回归和原场景连续10次通过、CMS429/429；未改运行时或OpenShip。该测试修复不等于网站业务功能已接通。
